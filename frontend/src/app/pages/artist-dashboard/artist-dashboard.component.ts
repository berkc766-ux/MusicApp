    ...
    <div class="pb-16">
      ...
      <div class="mb-6">
        <div *ngIf="artistProfiles.length > 1" class="flex items-center gap-3 mb-4">
          <svg viewBox="0 0 24 24" class="h-5 w-5 fill-blue-400 flex-shrink-0">
            <path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/>
          </svg>
          <p class="text-blue-400 text-sm font-semibold">Manager Mode — {{ artistProfiles.length }} artist profiles</p>
        </div>
        ...
        <div *ngIf="artistProfiles.length > 1" class="flex items-center gap-3 mb-5">
          ...
          <select [(ngModel)]="activeArtistIndex" (ngModelChange)="switchArtist($event)"
            class="bg-neutral-800 border border-neutral-700 text-white rounded-full px-4 py-2 text-sm focus:outline-none focus:border-blue-400 transition flex-1 max-w-xs">
      ...
      <div class="flex items-end gap-6 mb-8 mt-2 bg-gradient-to-b from-blue-900/40 to-transparent p-6 rounded-xl">
        <div class="h-36 w-36 bg-gradient-to-br from-blue-700 to-blue-950 rounded-full flex items-center justify-center flex-shrink-0 shadow-2xl">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-xs text-blue-400 font-semibold uppercase tracking-widest mb-1">🎤 Artist</p>
          ...
          <div *ngIf="!artist && !loading" class="mt-3 bg-yellow-500/10 border border-yellow-500/40 rounded-xl p-4 max-w-md">
            ...
            <div *ngIf="showProfileSetup" class="space-y-2">
              ...
              <p *ngIf="profileMsg" class="text-xs" [class]="profileSuccess ? 'text-blue-400' : 'text-red-400'">{{ profileMsg }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div *ngIf="artist" class="mb-6">
        ...
        <div *ngIf="showAddProfile" class="mt-3 bg-neutral-900 border border-neutral-700 rounded-xl p-4 max-w-md space-y-2">
          ...
          <input type="text" [(ngModel)]="newProfileStageName" placeholder="Stage name *"
            class="w-full bg-neutral-800 border border-blue-700/50 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-blue-400">
          ...
          <div class="flex gap-2 pt-1">
            <button (click)="addArtistProfile()" [disabled]="!newProfileStageName || addingProfile"
              class="bg-blue-500 text-black text-sm font-bold px-4 py-2 rounded-full hover:bg-blue-400 transition disabled:opacity-50">
          ...
          <p *ngIf="addProfileMsg" class="text-xs" [class]="addProfileSuccess ? 'text-blue-400' : 'text-red-400'">{{ addProfileMsg }}</p>
        </div>
      </div>
      ...
      <div class="flex gap-6 flex-col lg:flex-row">
        ...
        <div class="flex-1">
          <div class="flex items-center justify-between mb-4">
            ...
            <button (click)="showCreateAlbum = !showCreateAlbum" *ngIf="artist"
              class="bg-blue-500 text-black text-sm font-bold px-4 py-1.5 rounded-full hover:bg-blue-400 transition flex items-center gap-1">
              <span>+</span> New Album
            </button>
          </div>
            ...
            <div class="space-y-3">
              ...
              <div class="flex gap-2">
                <button (click)="createAlbum()" [disabled]="!newAlbum.title || !newAlbum.release_year || creatingAlbum"
                  class="bg-blue-500 text-black font-bold px-4 py-2 rounded-full text-sm hover:bg-blue-400 transition disabled:opacity-50">
              ...
              <p *ngIf="albumMsg" class="text-sm" [class]="albumSuccess ? 'text-blue-400' : 'text-red-400'">{{ albumMsg }}</p>
            </div>
          </div>
        ...
        <div *ngIf="activeAlbum" class="w-80 flex-shrink-0">
          <div class="bg-neutral-900 border border-blue-700 rounded-xl p-5 sticky top-4">
            <div class="flex justify-between items-center mb-4">
              <h3 class="text-white font-bold text-sm">Add Song to<br><span class="text-blue-400">{{ activeAlbum.title }}</span></h3>
            ...
            <div *ngIf="addSongTab === 'new'" class="space-y-3">
              <input type="text" [(ngModel)]="newSong.title" placeholder="Song title *"
                class="w-full bg-neutral-800 border border-neutral-700 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-blue-400">
              <input type="number" [(ngModel)]="newSong.duration_sec" placeholder="Duration (seconds)"
                class="w-full bg-neutral-800 border border-neutral-700 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-blue-400">
              <div class="flex gap-2">
                <select [(ngModel)]="newSong.category_id"
                  class="flex-1 bg-neutral-800 border border-neutral-700 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-blue-400">
              ...
              <label class="flex items-center gap-2 text-sm text-neutral-400 cursor-pointer">
                <input type="checkbox" [(ngModel)]="newSong.is_explicit" class="accent-blue-500">
                Explicit content
              </label>
              <button (click)="addSongToAlbum()" [disabled]="!newSong.title || addingSong"
                class="w-full bg-blue-500 text-black font-bold py-2 rounded-full text-sm hover:bg-blue-400 transition disabled:opacity-50">
                {{ addingSong ? 'Publishing...' : '🎵 Publish Song' }}
              </button>
              <p *ngIf="songMsg" class="text-sm" [class]="songSuccess ? 'text-blue-400' : 'text-red-400'">{{ songMsg }}</p>
            </div>

            <!-- Tab: Existing Song -->
            <div *ngIf="addSongTab === 'existing'">
              <input type="text" [(ngModel)]="existingSongSearch" placeholder="Search your songs..."
                class="w-full bg-neutral-800 border border-neutral-700 text-white rounded-md px-3 py-2 text-sm focus:outline-none focus:border-blue-400 mb-3">
              <div class="max-h-72 overflow-y-auto space-y-1">
                <div *ngFor="let song of filteredExistingSongs"
                  ...
                  <span class="text-blue-400 text-xs font-bold opacity-0 group-hover:opacity-100 transition ml-2">+ Add</span>
                </div>
                <p *ngIf="filteredExistingSongs.length === 0" class="text-neutral-600 italic text-xs px-3 py-2">No other songs found.</p>
              </div>
              <p *ngIf="existingSongMsg" class="text-sm mt-2" [class]="existingSongSuccess ? 'text-blue-400' : 'text-red-400'">{{ existingSongMsg }}</p>
            </div>
